...
### Fixed
- Mutant deduplication is now position-aware (replicate mutago #103). The dedup key hashed only the original node text and the replacement, so identical mutations anywhere in the run collapsed into one — every `0` → `1` literal in a codebase was tested exactly once, inflating MSI and hiding untested code. The key now includes the file and the edit's byte offsets, so mutants are deduplicated only when they produce byte-identical edits at the same location in the same file. **Note: `--blacklist` checksums generated by previous versions are invalidated** and must be regenerated.
...
- Mutant test runs now pass `-vet=off` to `go test` by default (replicate mutago #106). `go test` runs a `go vet` subset that exits 1 on any diagnostic, so a mutant that merely tripped vet (such as `expression/logical` producing a suspect `&&`) was reported as KILLED even though no test failed. Pass `-vet` via `--test-flags` to re-enable it; the baseline pre-flight and coverage runs (original code) still run vet.
...
### Fixed
- `--git-diff-lines` now diffs against the **merge-base** of the diff base and the current branch instead of the tip of the base branch. Previously, when a feature branch was behind its target, a plain two-dot `git diff <base>` attributed commits that had landed on the base after the branch point to the feature branch, so go-mutesting mutated lines the developer never touched. The filter now matches exactly what the pull request shows, while still including uncommitted working-tree changes.
...
[v2.7.0]: https://github.com/jonbaldie/go-mutesting/compare/v2.6.16...v2.7.0
[v2.7.1]: https://github.com/jonbaldie/go-mutesting/compare/v2.7.0...v2.7.1
[v2.7.2]: https://github.com/jonbaldie/go-mutesting/compare/v2.7.1...v2.7.2
...
[v2.8.2]: https://github.com/jonbaldie/go-mutesting/compare/v2.8.1...v2.8.2
[v2.8.3]: https://github.com/jonbaldie/go-mutesting/compare/v2.8.2...v2.8.3
[v2.8.4]: https://github.com/jonbaldie/go-mutesting/compare/v2.8.3...v2.8.4
[v2.8.5]: https://github.com/jonbaldie/go-mutesting/compare/v2.8.4...v2.8.5
[v2.8.6]: https://github.com/jonbaldie/go-mutesting/compare/v2.8.5...v2.8.6
