...
2. Build `/tmp/go-mutesting` and run the full self-mutation command/package list in `CLAUDE.md`; both MSI gates must pass (exit `0`; exit `4` is a gate failure).
...
7. Merge to `master`, tag the next semver version, and create a succinct GitHub release.