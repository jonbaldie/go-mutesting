...
6. **Commit and push** — fix forward only. No `--force-push` and no `--amend` on published commits. If a hook or check fails, fix it in a new commit.
7. **Watch CI** — wait for the Actions run to go green before merging into master.
