...
5. **Commit and push** — fix forward only. No `--force-push` and no `--amend` on published commits. If a hook or check fails, fix it in a new commit.
6. **Watch CI** — wait for the Actions run to go green before merging into master.
7. **Merge to master** — then push master.
8. **Tag and release** — pick the next semver tag. Create a GitHub release page: following prior release pages, use a very succinct style and 8th grade English.
